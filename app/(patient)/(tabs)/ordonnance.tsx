...
export default function PatientOrdonnance() {
  ...
  // Recharge à chaque fois que l'écran redevient visible (ex : retour depuis le
  // détail après avoir démarré un traitement) → le statut affiché est à jour.
  useFocusEffect(
    useCallback(() => {
      fetchPrescriptions();
    }, [])
  ...
  return (
    <SafeAreaView className="flex-1 bg-slate-50" edges={['left', 'right', 'bottom']}>
      <AppHeader subtitle="Mes ordonnances" />
      {/* Header */}
